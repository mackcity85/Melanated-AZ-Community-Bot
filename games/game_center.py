...
def console_home_keyboard():
    rows=[[InlineKeyboardButton("🥊 FIGHTING",callback_data="games_genre_all_fighting"),InlineKeyboardButton("🏆 SPORTS",callback_data="games_genre_all_sports")]]
...
async def genre_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer()
    rows=[[InlineKeyboardButton(f"{g['icon']} {g['name']} — {g['system_name']}",url=f"https://melanatedaz.onrender.com/real-games/play/{g['game_id']}")] for g in games]
    rows.append([InlineKeyboardButton("⬅️ Game Center",callback_data="games_home")])
...
async def system_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer(); rows=[[InlineKeyboardButton(f"{g['icon']} {g['name']}",url=f"https://melanatedaz.onrender.com/real-games/play/{g['game_id']}")] for g in games]
    rows += [[InlineKeyboardButton(f"{g['icon']} {g['name']} — {g['genre'].capitalize()}",url=f"https://melanatedaz.onrender.com/real-games/play/{g['game_id']}")] for g in genre_games]
