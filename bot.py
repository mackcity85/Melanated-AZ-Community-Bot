...
async def register_all_startup_jobs(application):
    ...
    application.job_queue.run_repeating(
        community_security_monitor, interval=INACTIVITY_CHECK_HOURS * 60 * 60,
        first=60, name="community-security-monitor")
    application.job_queue.run_once(
        send_monthly_intro_reminders, when=15, name="monthly-intro-reminders-initial")
    application.job_queue.run_repeating(
        send_monthly_intro_reminders, interval=60 * 60, first=60 * 60,
        name="monthly-intro-reminders")

    application.job_queue.run_once(
        _run_games_topic_pin_maintenance, when=5, name="games-topic-pins-startup")
    ...
    application.job_queue.run_once(
        repair_active_raffle_post, when=10, name="one-time-raffle-topic-repair")
...
async def post_init(application):
    ...
    if main:
        try:
            ...
            logger.info("Community bot membership: status=%s | can_restrict_members=%s | can_delete_messages=%s",
                        member.status,getattr(member,"can_restrict_members",None),
