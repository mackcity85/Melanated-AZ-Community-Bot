...
#
# Uses Telegram deep link registration.
# ==========================================================
...
def raffle_message(raffle):
    ...
    return (
        ...
        "⚠️ **IMPORTANT:** Your raffle entry is NOT "
        "active until MelanatedAZ verifies and "
        "approves your payment."
    )


# ==========================================================
# PRIVATE RAFFLE REGISTRATION MESSAGE
# ==========================================================
...
def private_raffle_message(raffle):
    ...
    return (
        ...
        "Your entry is **NOT valid or active** "
        "until payment has been verified and "
        "approved by MelanatedAZ."
    )
...
async def handle_raffle_setup(
    ...
    if not prize or not price:
        ...
        await message.reply_text(
            "❌ Both the prize and entry price "
            "are required.\n\n"
            "Example:\n"
            "`$100 Cash Prize | $5`",
            parse_mode="Markdown",
...
async def create_pending_raffle(
    ...
    if existing:
        ...
        await message.reply_text(
            "⚠️ There is already an active or "
            "pending raffle.\n\n"
    ...
    await message.reply_text(
        ...
        "The raffle has NOT been posted "
        "to the group yet.\n\n"
    ...
    if not approval_sent:
        ...
        await message.reply_text(
            "⚠️ The raffle was created, but I "
            "could not send the approval controls.\n\n"
            "Check RAFFLE_CHAT_ID and make sure "
            "the bot can send messages there."
        )


# ==========================================================
# PRIVATE RAFFLE START / REGISTRATION
# ==========================================================
...
async def raffle_private_start(
    ...
    logger.info(
        "Raffle start payload received from "
        "user %s: %s",
        user.id,
        payload,
    )
    ...
    if not payload.startswith(
        ...
        await message.reply_text(
            ...
            "Please use the **REGISTER WITH "
            "MELANATED AZ** button from the "
            "active raffle in the group.",
            parse_mode="Markdown",
        )
    ...
    logger.info(
        "Raffle registration displayed to "
        "user %s for raffle %s",
        user.id,
        raffle_id,
    )
...
async def admin_payment_button(
    ...
    if action == "deny":
        ...
        try:
            ...
            logger.warning(
                "Unable to edit admin denial message.",
                exc_info=True,
            )
        ...
        try:
            ...
            await context.bot.send_message(
                ...
                text=(
                    ...
                    "MelanatedAZ was unable to "
                    "verify your payment, so your "
                    "raffle entry was not activated."
                ),
...
# ==========================================================
# RECOVER / REPOST EXISTING RAFFLE
#
# IMPORTANT:
# This does NOT create a raffle.
#
# It uses the existing database record.
#
# If the original Telegram message was deleted,
# the existing raffle is reposted and the database
# receives the new message ID.
# ==========================================================
...
# RECOVERY BEHAVIOR:
#
# 1. Load existing raffle from database.
# 2. Never create a new raffle.
# 3. Use stored chat/message ID.
# 4. If message exists, edit countdown.
# 5. If Telegram says message is gone,
#    repost SAME raffle.
# 6. Save new message ID.
# 7. Keep same raffle ID, prize, price,
...
async def update_raffle_countdown(
    ...
    try:
        ...
        if datetime.utcnow() >= expiration:
            ...
            if (
                ...
                try:
                    ...
                    logger.warning(
                        "Could not edit expired raffle "
                        "message for raffle %s: %s",
                        raffle_id,
                        error,
                    )
        ...
        # ==================================================
        # VERIFY STORED MESSAGE INFORMATION
        # ==================================================
        ...
        if (
            ...
            logger.warning(
                ...
                "message ID. Recovering existing raffle.",
                raffle_id,
            )
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    "Telegram message for raffle %s "
                    "is no longer available. "
                    "Recovering existing raffle "
...
# ==========================================================
# RECOVER ACTIVE RAFFLE COUNTDOWN
#
# This helper can be called by bot.py on startup.
#
# It finds the existing active raffle and starts
# its countdown without creating anything.
# ==========================================================

async def recover_active_raffle_job(
    context: ContextTypes.DEFAULT_TYPE,
):
    ...
    logger.info(
        "Found existing active raffle %s during "
        "startup recovery.",
        raffle_id,
    )

    # ------------------------------------------------------
    # Check expiration before scheduling.
    # ------------------------------------------------------
    ...
    try:
        ...
        logger.exception(
            "Unable to validate expiration for "
            "raffle %s.",
            raffle_id,
        )
    ...
    # ------------------------------------------------------
    # IMPORTANT:
    # Keep the requested 5-minute delay.
    # ------------------------------------------------------
