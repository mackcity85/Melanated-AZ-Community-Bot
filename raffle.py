...
async def enter_raffle(update, context, raffle_id):
    ...
    # Send the same raffle-entry notification to each configured admin.
    for admin_id in ADMIN_IDS:
        try:
            await context.bot.send_message(
    ...
    # Send the SAME notification to the Admin Group.
    # Free raffles are auto-approved, so they receive no approval buttons.
    try:
        admin_group_id = int(os.environ.get("ADMIN_GROUP_ID", "0") or "0")
    except (TypeError, ValueError):
        admin_group_id = 0
    ...
    if admin_group_id:
        try:
            ...
            logger.info(
                "RAFFLE ENTRY NOTIFICATION | entry=%s | raffle=%s | "
                "destination=ADMIN_GROUP:%s | SENT | message=%s",
                entry_id,
                raffle_id,
                admin_group_id,
                sent_group.message_id,
            ...
            logger.exception(
                "RAFFLE ENTRY NOTIFICATION | entry=%s | raffle=%s | "
                "destination=ADMIN_GROUP:%s | FAILED",
                entry_id,
                raffle_id,
                admin_group_id,
            )
