...
# Public raffle updates are posted in the Games topic.
# Arizona stays on Mountain Standard Time (UTC-7) year-round.
RAFFLE_GAMES_TOPIC_ID = int(__import__("os").environ.get("RAFFLE_GAMES_TOPIC_ID", "8809") or "8809")
...
async def track_entry_message(entry_id, sent_message, kind="entry"):
    """Track a temporary raffle-entry message for later cleanup."""
    if not sent_message:
        return
    try:
        record_raffle_entry_message(
        ...
        logger.exception("Could not track raffle entry message | entry=%s", entry_id)

async def post_public_raffle_status(context, raffle_id):
    """Post a fresh public Raffle Status message without changing the raffle."""
    raffle = get_raffle(raffle_id)
    if not raffle or raffle.get("status") != "active":
        return None

    approved = get_approved_entries(raffle_id)
    pending = get_pending_entries(raffle_id)
    text = (
        "🎟️ <b>RAFFLE STATUS</b>\n\n"
    ...
    try:
        ...
        logger.exception("Could not repost raffle status | raffle=%s", raffle_id)
        return None

async def send_daily_raffle_status(context: ContextTypes.DEFAULT_TYPE):
    """Post the daily public raffle status at 8:00 AM MST when a raffle is active."""
...
def start_daily_raffle_status(application):
    """Schedule the daily 8:00 AM MST raffle status update."""
    if not application.job_queue:
        logger.warning("Daily raffle status unavailable: JobQueue not installed.")
        return

    # Remove an older copy if the application is reinitialized.
    ...
    logger.info(
        "Daily raffle status scheduled for 8:00 AM MST | chat=%s | topic=%s",
        RAFFLE_CHAT_ID,
        RAFFLE_GAMES_TOPIC_ID,
    )
...
async def cleanup_approved_entry_messages(context):
    """Remove every tracked temporary message for an approved entry."""
    ...
    for record in records:
        try:
            ...
            try:
                ...
                logger.exception("Could not mark raffle message cleanup record %s.", record["id"])

    # Keep approved entries in SQLite for the winner draw. Only the temporary
...
def schedule_approved_entry_cleanup(entry_id, raffle_id, context):
    ...
    cleanup_at = (datetime.utcnow() + timedelta(seconds=APPROVED_ENTRY_CLEANUP_SECONDS)).isoformat()
    try:
        schedule_raffle_entry_message_cleanup(entry_id, cleanup_at)
    except Exception:
        logger.exception("Could not persist raffle cleanup deadline | entry=%s", entry_id)

    if not getattr(context, "job_queue", None):
        logger.error("Job queue unavailable; raffle cleanup not scheduled | entry=%s", entry_id)
        return

    name = f"raffle-entry-cleanup-{entry_id}"
    for job in context.job_queue.get_jobs_by_name(name):
        job.schedule_removal()
    context.job_queue.run_once(
        cleanup_approved_entry_messages,
        APPROVED_ENTRY_CLEANUP_SECONDS,
        data={"entry_id": int(entry_id), "raffle_id": int(raffle_id)},
        name=name,
    )
...
async def enter_raffle(update, context, raffle_id):
    ...
    sent_confirmation = await query.message.reply_text(
    ...
    await track_entry_message(entry_id, sent_confirmation, "pending_confirmation")
    keyboard = InlineKeyboardMarkup([[
    ...
    for admin_id in ADMIN_IDS:
        try:
            ...
            await track_entry_message(entry_id, sent_admin, "admin_pending")
        except TelegramError:
            logger.warning("Could not notify admin %s.", admin_id)
...
async def approve_entry_callback(update, context, entry_id):
    ...
    schedule_approved_entry_cleanup(entry_id, int(entry["raffle_id"]), context)
...
async def pending_entries(update, context):
    ...
    for entry in pending:
        ...
        try:
            sent_pending = await target.reply_text(
                text, reply_markup=keyboard, parse_mode=ParseMode.HTML
            )
            await track_entry_message(entry["id"], sent_pending, "admin_pending_list")
        except TelegramError:
