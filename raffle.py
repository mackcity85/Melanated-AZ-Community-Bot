...
try:
    ...
    #
    # If the admin message itself was deleted,
    # don't let that prevent the raffle from working.
    # --------------------------------------------------
    ...
    try:
        ...
        logger.warning(
            "Could not edit raffle approval "
            "message for raffle %s.",
            raffle_id,
            exc_info=True,
        )
    ...
    try:
        ...
        logger.exception(
            "Unable to report raffle posting error."
        )
```
...
if action == "approve":
    ...
    try:
        ...
        logger.warning(
            "Could not edit payment approval message.",
            exc_info=True,
        )
...
if action == "deny":
    ...
    try:
        ...
        logger.warning(
            "Could not edit payment denial message.",
            exc_info=True,
        )
...
# COUNTDOWN UPDATE / MESSAGE RECOVERY
...
# If the original Telegram raffle message was deleted,

# it reposts the EXISTING raffle using the SAME raffle ID.
...
# are all preserved in the database.
...
if not raffle:
    ...
    logger.error(
        "Raffle %s could not be found in database.",
        raffle_id,
    )
...
if raffle["status"] != "active":
    ...
    logger.info(
        "Raffle %s is no longer active: %s",
        raffle_id,
        raffle["status"],
    )
...
try:
    ...
    if chat_id and message_id:
        ...
        try:
            ...
            # --------------------------------------------------
            # ONLY RECOVER WHEN TELEGRAM SAYS THE MESSAGE
            # DOES NOT EXIST.
            # --------------------------------------------------
            ...
            if (
                ...
                logger.exception(
                    "Unable to update raffle %s countdown.",
                    raffle_id,
                )
            ...
            logger.warning(
                "Raffle %s message %s no longer exists. "
                "Recovering existing raffle message.",
                raffle_id,
                message_id,
            )
    ...
    #
    # THIS DOES NOT CALL create_raffle().
    #
    # IT ONLY CREATES A NEW TELEGRAM MESSAGE FOR THE
    # EXISTING DATABASE RAFFLE.
    # ==================================================

    if not RAFFLE_CHAT_ID:

        logger.error(
            "Cannot recover raffle %s because "
            "RAFFLE_CHAT_ID is not configured.",
            raffle_id,
        )
    ...
    try:
        ...
        logger.exception(
            "Unable to recover Telegram message "
            "for existing raffle %s.",
            raffle_id,
        )
    ...
    try:
        ...
        set_raffle_post(
            raffle_id,
            RAFFLE_CHAT_ID,
            recovered_message.message_id,
        )

        logger.info(
            "SUCCESS: Recovered existing raffle %s. "
        ...
        logger.exception(
            "Raffle %s was reposted, but the new "
            "Telegram message ID could not be saved.",
            raffle_id,
        )
