...
async def _run_social_media_profile_recovery(context):
    try:
        ...
        logger.info("Social Media profile recovery COMPLETE | recreated=%s", recovered)
    except Exception:
        ...
        async def patched_post_init(app):
            ...
            try:
                if app.job_queue:
                    ...
                    logger.info("Games-topic pin startup check scheduled | chat=%s topic=%s", -1002697105809, 8809)
            except Exception:
            ...
            # Member social profiles remain recoverable. Recovery only creates
            # a new profile when the stored Telegram message is actually gone.
            try:
                if app.job_queue:
                    for name in ("social-media-profile-recovery", "social-media-profile-recovery-startup"):
                        for job in app.job_queue.get_jobs_by_name(name):
                            job.schedule_removal()
                    app.job_queue.run_once(_run_social_media_profile_recovery, when=10, name="social-media-profile-recovery-startup")
                    app.job_queue.run_repeating(_run_social_media_profile_recovery, interval=600, first=600, name="social-media-profile-recovery")
                    logger.info("Social Media member profile recovery enabled | startup=10s | interval=600s")
            except Exception:
                logger.exception("Social Media member profile recovery scheduling failed")
            ...
            try:
                ...
                if app.job_queue:
                    ...
                    logger.info("GUARANTEED intro startup enabled | cancelled_old_jobs=%s", cancelled)
            except Exception:
